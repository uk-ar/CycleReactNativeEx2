...
storiesOf("Browser", module).add("with full book", () => (
  <Book
...
storiesOf("Main", module).add("cycle", () => <CycleRoot />);

storiesOf("SearchBar", module).add("with loading", () => (
  <SearchBar
    lightTheme
    showLoadingIcon
...
storiesOf("SearchScene", module)
  ...
  .add("with no items", () => (
    <SearchScene
      ...
      selectedIndex={1}
      rejects={[]}
      searchHistory={[{ query: "foo" }, { query: "bar" }]}
      data={null}
    />
  ))
  .add("with class", () => (
...
storiesOf("Library", module).add("with plane", () => (
  <Library title="foo" subtitle="bar" />
));

storiesOf("LibraryList", module).add("with plane", () => (
  <LibraryList
    data={[
